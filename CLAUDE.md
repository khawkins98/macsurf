...
- **CW8 C89 rejects 4-arg calls to a 3-param macro** — `#define NSLOG(cat, level, fmt) do{}while(0)` causes `')' expected` when called as `NSLOG(cat, level, "fmt %s", val)`. CW8 does NOT silently discard extra macro arguments. The only valid approach is a real C varargs function: `static void noop_(const void*c, long l, const char*f, ...) { (void)c; (void)l; (void)f; }` + `#define NSLOG noop_`. Category tokens like `netsurf`, `llcache` etc. must be valid integer expressions — safe ones can be `#define`d as `0`, but `fetch`/`plot`/`layout`/`flex` CANNOT (they conflict with struct tags and variable names in the codebase). Fixed in fixes259 for `macsurf_prefix.h`.
