...
- **Locked config:** `qemu-system-ppc` 11 (native arm64), `-M mac99` (`via=pmu` normally; **drop `via=pmu` for the usb-tablet INIT**), `-cpu g4`, `-m 512` (HARD: never ≥1GB), `-g 1024x768x32`, `sungem` NIC, `-accel tcg,split-wx=on,tb-size=512`. No PPC accel exists on arm64 — pure single-thread TCG.
- **CodeWarrior 8 installed entirely host-side** (mount the qcow2's HFS+ volume via `hdiutil -imagekey diskimage-class=CRawDiskImage`, `ditto` the IDE folder + System Folder Items) — no installer, no serial. IDE launches.
- **Input reality:** QMP keyboard works; QMP mouse *clicks* do NOT register on OS 9 (polled HID misses instant down→up; usb-mouse ignores standalone button reports). Solutions: usb-tablet + `kanjitalk755` INIT (absolute clicks), or drive builds keyboard-only via CodeWarrior AppleEvents (Script Editor + Cmd-R; `Make` returns errors as data; project/access-paths via XML Import; "golden prefs" to kill the first-launch dialog).
