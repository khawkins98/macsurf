...
- **Build status:** the repo `MacSurf.mcp` is a CodeWarrior **XML project export** — open it with **File → Import Project** (NOT Open), saving the regenerated `.mcp` into the macos9 source folder so its relative paths resolve. Source is staged on the baseline disk at the expected `Back40:…` paths by `stage-on-bootvol.sh`. Import verified working; the first full Make is the remaining step.
- **Build images** live outside the repo at `~/macsurf-qemu/` (never committed).

## Known Gotchas

- **The `MacSurf.mcp` checked into git is a CodeWarrior XML project export, not a binary project.** Git strips resource forks, and CW binary `.mcp` projects live in the resource fork — so the repo copy is the XML form (531 `<FILE>` entries, relative paths). On a Mac, open it with **File → Import Project** (which regenerates a binary `.mcp`); **File → Open fails** with "Resource File Error 207008 — not a resource file". The regenerated `.mcp` must be saved in `browser/netsurf/frontends/macos9/` so the XML's relative `../../../` file refs resolve. The binary `.mcp` on the dev Mac remains the working project (per "Adding new .c files" above); the XML in git is the durable, diffable record of it.
