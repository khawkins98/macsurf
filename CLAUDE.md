...
- Last shipped fix: **fixes78** — **`<img>` content handler via QuickTime Graphics Importers.** New file [browser/netsurf/frontends/macos9/macos9_image.c](browser/netsurf/frontends/macos9/macos9_image.c) implements a single NetSurf `content_handler` that ingests bytes via `process_data` into a growing Mac `Handle`, builds a QuickTime data reference at `data_complete` (`PtrToHand` of the handle pointer with `HandleDataHandlerSubType`), asks QT for an importer via `GetGraphicsImporterForDataRef`, queries natural bounds via `GraphicsImportGetNaturalBounds`, and draws on `redraw` via `GraphicsImportSetGWorld` + `SetBoundsRect` + `Draw` directly into the active QD port. No NetSurf bitmap intermediate. The active port at redraw time is the offscreen GWorld set up by fixes77f, so QT renders straight into the back buffer and respects its clipRgn for hardware clipping. Registered MIME types: image/jpeg, image/jpg, image/pjpeg, image/png, image/x-png, image/gif, image/bmp, image/x-bmp, image/x-ms-bmp. The `image_init()` stub in `misc_stub.c` is removed; the real symbol now lives in `macos9_image.c` and is called from `netsurf_init` (desktop/netsurf.c:204) as it always was. **Mac-side TODO:** add `macos9_image.c` to MacSurf.mcp and ensure QuickTime is linked (CarbonLib's QuickTime support is sufficient on OS 9 — typically already present). WebP / AVIF runtime `RegisterComponent` (GraphicsImporterComponentType + `'WEBP'` subtype) is queued as a separate phase 2 sprint.
