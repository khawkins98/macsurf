...
- Last shipped fix: **fixes311** — Structural build recovery. (1) `macsurf_prefix.h` defines `restrict` as empty, blocks more MSL C++ headers, and includes core POSIX headers (`stddef.h`, `stat.h`, `fcntl.h`, `time.h`, `stdbool.h`) to stabilize the parse environment. (2) `log.h` converted to standard C99 `__VA_ARGS__` to resolve widespread syntax errors. (3) `bloom.h` `restrict` usage removed. (4) `stat.h` shim forwards to MSL on CW8. (5) `mac_file_io.c` includes `<fcntl.h>`. (6) `sys/time.h` includes `<stddef.h>`.
