...
- **Build status:** the repo `MacSurf.mcp` is a **hand-maintained manifest**, not a genuine CW export — CW's Import rejects it (schema mismatch; see Known Gotchas). Source is staged on the baseline disk at the expected `Back40:…` paths by `stage-on-bootvol.sh`. Remaining: capture a genuine CW8 project export from the guest, convert the manifest to that schema (`manifest-to-mcpxml` converter, planned), import, then the first full Make.
