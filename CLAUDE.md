...
- Last shipped fix: **fixes141** — **standard `transform` property bridge.** Three-file change in libcss: [propstrings.h](browser/libcss/src/parse/propstrings.h) gains a `TRANSFORM` enum entry (LAST_PROP bumped from `WORD_WRAP` to `TRANSFORM`), [propstrings.c](browser/libcss/src/parse/propstrings.c) gains an `SMAP("transform")` entry in lockstep, and [properties.c](browser/libcss/src/parse/properties/properties.c) gains a `property_handlers[]` entry pointing at the existing `css__parse_macsurf_transform`. The parser hardcodes `CSS_PROP_MACSURF_TRANSFORM` in its bytecode, so both standard `transform` and vendor `-macsurf-transform` emit bit-identical bytecode that the existing fixes73 cascade / select / redraw / plotter path consumes unchanged. Pattern mirrors `word-wrap` -> `overflow-wrap` alias one line up. Cascade tie-break: since both names target the same internal slot, the LATER declaration wins by normal source order — `transform: rotate(15deg)` after `-macsurf-transform: rotate(5deg)` resolves to 15deg, and vice versa. No new compositor, no new cascade handler, no QuickDraw plotter touched. Test probes T1–T6 in `tests/css/transform_standard.html`. Deferred: `transform-origin`, `matrix()`, `skew()`, 3D transforms, individual `rotate` / `translate` / `scale` properties.
