...
- Last shipped fix: **fixes305a** — `macsurf_prefix.h` define `MACSURF_DEBUG 1` (gated on `#ifndef MACSURF_RELEASE`). The file-backed log channel regressed during the fixes260–304 sprint: `MACSURF_DEBUG` was never set, so `macsurf_debug_log_init()` reduced to the empty release stub and every `MS_LOG()` call compiled out. Init call at [main.c:191](browser/netsurf/frontends/macos9/main.c#L191) was already wired (fixes152) — only the macro define was missing. Same regression class as fixes149→fixes152, different vector (gated-init instead of missed-init). See "Regression Audit Checklist" for the new step 2 that catches this case. Predecessor (the 38-fix sprint summarised in [docs/fixes-260-304-sprint.md](docs/fixes-260-304-sprint.md)) starts at fixes266 below.
