...
- **v0.4.6 IMAGES on G4 / OS 9.2.2 (2026-05-16).** Reached at fixes78k. PNG / GIF / BMP / TIFF / JPEG all render via QuickTime Graphics Importers. Verified on `advanced.html` PROBE I1–I5: MPLS logo renders identically across PNG (transparency), GIF (palette), BMP (227KB uncompressed), TIFF (Mac-native), and the 286KB garykaija.jpg photo decodes cleanly.
- Last shipped fix: **fixes78k** — **`<img>` content handler via QuickTime Graphics Importers + NetSurf bitmap dispatch.** New file [browser/netsurf/frontends/macos9/macos9_image.c](browser/netsurf/frontends/macos9/macos9_image.c) implements a single NetSurf `content_handler`. Architecture (after the fixes78j refactor):
