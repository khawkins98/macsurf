...
## QEMU as a Build & Test Tool (experimental — fork branch `feat/qemu-os9-build-automation`)

A QEMU PowerPC + Mac OS 9.2.1 harness on the Apple Silicon dev Mac, aimed at an **off-hardware build-and-test loop** (compile the CodeWarrior project + exercise the fetch path against the proxy without round-tripping to the G3/G4 each iteration). Unlike SheepShaver, QEMU here has **working networking** (SLIRP; guest reaches the host proxy at `10.0.2.2:8765`). Scripts + docs in [tools/qemu/](tools/qemu/) — start at [tools/qemu/README.md](tools/qemu/README.md) (how to use everything); details in [SETUP.md](tools/qemu/SETUP.md), [docs/INPUT.md](tools/qemu/docs/INPUT.md), and **[docs/GOTCHAS.md](tools/qemu/docs/GOTCHAS.md)** (harness-specific gotchas — the emulation-side counterpart to the Known Gotchas below).

- **Same caveat as SheepShaver:** smoke/integration tier, NOT a replacement for real-hardware verification (wheel/scroll/USB Overdrive, the 9.1↔9.2.2 font delta still need a real Mac).
- **Locked config:** `qemu-system-ppc` 11 (native arm64), **`-M mac99` (no via=pmu) + `-device usb-tablet`** (required for scripted clicks — see next bullet), `-cpu g4`, `-m 512` (HARD: never ≥1GB), `-g 1024x768x32`, `sungem` NIC, `-accel tcg,split-wx=on,tb-size=512`. No PPC accel exists on arm64 — pure single-thread TCG.
- **The guest GUI is fully drivable by an agent** (keyboard + absolute mouse clicks + screenshots via QMP). Plain OS 9 ignores QMP clicks; the fix is the **`kanjitalk755` "USB Tablet INIT"** in the guest's `System Folder:Extensions:` + the machine config above. Already installed in the working baseline disk. Menus need press-drag (`vm.py pressdrag`), not click-then-click. Full notes: [docs/INPUT.md](tools/qemu/docs/INPUT.md).
- **CodeWarrior 8 installed entirely host-side** (mount the qcow2's HFS+ volume via `hdiutil -imagekey diskimage-class=CRawDiskImage`, `ditto` the IDE folder + System Folder Items) — no installer, no serial. IDE launches and is scriptable (AppleEvents / GUI).
- **Build status:** the repo `MacSurf.mcp` is a **hand-maintained manifest**, not a genuine CW export — CW's Import rejects it (schema mismatch; see Known Gotchas). Source is staged on the baseline disk at the expected `Back40:…` paths by `stage-on-bootvol.sh`. **The converter exists and works:** `tools/codewarrior/manifest-to-mcpxml.py` generates a genuine CW8-importable XML (rename map for 25 stale manifest entries, `dispatch.c`→`s_dispatch.c` twin correction, stale-twin warnings). **Host-side precheck gate also exists:** `tools/precheck.sh` (clang C89 gate over the manifest, ~15s) — run it before any guest build. Remaining: import the generated XML in the guest, then the first full Make. **Driving document: [tools/qemu/ROADMAP.md](tools/qemu/ROADMAP.md).**
- **Build images** live outside the repo at `~/macsurf-qemu/` (never committed).

## Known Gotchas

- **The `MacSurf.mcp` checked into git is a hand-maintained XML manifest, not an openable/importable CodeWarrior project.** Git strips resource forks (CW binary projects live in the resource fork), so the repo copy is XML — but it is written in CW-export-*inspired* style, not CW's actual schema: it lacks the `<?codewarrior exportversion?>` processing instruction, uses `FILEFLAGS` values (`compile`/`link`) that aren't CW vocabulary, has no `PATHFORMAT` elements, and uses a flat made-up access-path form. Consequently **File → Open fails** ("Resource File Error 207008 — not a resource file") and **File → Import Project also fails** ("Error importing XML … near line 367"). The binary `.mcp` on the dev Mac remains the only working project; the git XML is the durable, diffable *record* of the file list. To make it importable, use the converter `tools/codewarrior/manifest-to-mcpxml.py` (genuine CW8 schema, rename map for stale entries, stale-twin detection) — workflow and status in [tools/qemu/ROADMAP.md](tools/qemu/ROADMAP.md).
