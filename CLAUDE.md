...
- **`gap` / `row-gap` not parsed** — 76 uses in MacTrove drop silently. Queued fixes148 (deferred from fixes147 after scope audit: adding a new libcss property touches ~17 files including the bit-packed `autogenerated_computed.h` struct, plus ~50 lines in `layout_flex.c`; needs a dedicated round).
...
- Last shipped fix: **fixes147** — two commits in one zip:
  - **Commit A (scroll-bar click crash, high confidence):** Clicking anywhere in a scroll bar crashed at `PowerPC illegal instruction at 00000008, LR=00000004` with CurApName `CodeWarrio...`. Root cause: the UPP macro override in [window.c](browser/netsurf/frontends/macos9/window.c) — `#define NewControlActionUPP(proc) ((ControlActionUPP)(proc))` — was passing a raw PPC function pointer where CarbonLib's `TrackControl` expects a MixedMode-compatible routine descriptor. CarbonLib's `InvokeControlActionUPP` read scroll_action's entry-code bytes as descriptor fields, resolved a routine address of 0, and executed `bl 0`. The LR=4 / PC=8 signature matches exactly: `bl` at address 0 set LR=4, low-memory values decoded as NOP-like instructions until first illegal opcode at PC=8. Fix: drop the action UPP entirely. `TrackControl(ctrl, pt, NULL)` — no UPP dispatch. `kControlScrollBarLiveProc = 386` live-tracks the thumb via the CDEF; `GetControlValue` on return holds the final drag position. Arrow / page clicks scroll by one step per click based on the `ControlPartCode` the caller captured from `FindControl`. Trade-off: no auto-repeat on arrow-hold (keyboard arrows still repeat via autoKey). See the new "UPP macro override on CarbonLib" entry in Known Gotchas.
...
- **fixes148 — `gap` / `row-gap` parsing and layout consumption.** 76 uses in MacTrove currently silent. Fixes text-overlap complaints. Deferred from fixes147 after scope audit: adding a new libcss property properly requires editing ~17 files (enum, parse, select, propset/propget macros, autogenerated computed-style struct with bit-packed offsets, dispatch table, opcodes, hash, etc.) plus ~50 lines in `layout_flex.c` for flex-direction-aware gap distribution. The `autogenerated_computed.h` bit-packing is the real risk: new field means recalculating offsets, which breaks every propset/propget downstream if done wrong. Worth a dedicated round with a proper bit-packing audit — should not be rushed alongside another fix.
- **fixes149 — flex alignment reads in `layout_flex.c`.** libcss computes `justify-content` / `align-content` / `order` / `column-gap`; layout ignores them. Follow the `lh__box_align_self` pattern. (column-gap is already computed but unread; this round consumes it in addition to wiring the existing accessors.)
