...
- **P4 — `counter-increment` / `counter-reset` (NOT consumed).** Auto-numbered headings, ordered lists with custom numbering, TOCs all silently broken.
...
- **fixes137 — counters (P4, medium effort).** `counter-increment`, `counter-reset`, and `content: counter(name)` resolution. New `redraw_counters.c` helper invoked from `html_redraw_box`.
