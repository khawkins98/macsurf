...
- Last shipped fix: **fixes135c** — **text-overflow ellipsis via paint-after.** Different architecture from the reverted fixes135b. Original text draws normally (overflow:hidden clips it at the container edge), then two extra plot calls paint an ellipsis on top of the rightmost slice: a background-coloured rect over `[clip->x1 - marker_w .. clip->x1]` followed by a `…` text call at `(clip->x1 - marker_w, y)`. No text-buffer mutation, no binary search, no UTF-8 byte boundary arithmetic, no stack buffer — sidesteps the whole bug class that broke fixes135b. Single file: redraw.c.
