...
**Current fix round:** v1.3.1 "Forward, refined" — **multi-curve ECDHE lands in TLS 1.3.** macTLS now offers X25519 (`0x001D`), secp256r1 / P-256 (`0x0017`), and secp384r1 / P-384 (`0x0018`) in `supported_groups`. The `key_share` carries an X25519 public key by default; servers that disallow X25519 (FIPS zones, certain XenForo-on-nginx configs, some Cloudflare strict zones) respond with HelloRetryRequest naming a NIST curve, and macTLS now rebuilds + resends ClientHello exactly once with a fresh key share on the requested curve. A latent HRR infinite-loop bug (was being handled inline via WantRead) is closed by a clean `hrr_pending` state transition consumed at the next pump step. Hardware-verified: **68kmla.org/bb/ fully renders on the G3 iMac** — TLS 1.3 + P-384 via HRR, 2,759 box-tree nodes, 19 images, zero handshake failures across a session. X25519 baseline (mactrove, google, cloudflare, howsmyssl) unchanged. No MacSurf project file changes; transparent macTLS engine upgrade. Companion MacSurf fixes: **fixes317** (universal HTTPS↔HTTP fallback with per-host bounce-loop guard + single-slash URL repair), **fixes318** (CHUNKDIAG instrumentation for the pending Google Fonts chunked-stall capture).
**Predecessor:** v1.3 "Forward" (2026-05-29) — first native TLS 1.3 on Classic Mac OS, ever. Hand-written RFC 8446 handshake on BearSSL primitives. v1.2 "Sealed" (same day, ~6 hours earlier) — macEntropy v1.0 closed the entropy hole, POST forms wired, V1 download manager landed.
**Last release:** **MacSurf v1.3.1 "Forward, refined"** (2026-05-29). Full notes: [release-notes/MacSurf-1.3.1.md](release-notes/MacSurf-1.3.1.md). Predecessor v1.3 notes: [release-notes/MacSurf-1.3.md](release-notes/MacSurf-1.3.md). v1.2 notes: [release-notes/MacSurf-1.2.md](release-notes/MacSurf-1.2.md).
**Last hardware-accepted:** v1.3.1 multi-curve ECDHE (68kmla.org over P-384, 2026-05-29).
