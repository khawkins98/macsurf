...
**fixes191 shipped (six-feature theme-completion sprint, in addition
to the originally-planned tracks):**

| Sub | Feature | Approach |
|---|---|---|
| 191a | `inset` shorthand | cssh_css preprocessor expands `inset: A [B [C [D]]]` to top/right/bottom/left longhands with `!important` propagated. No libcss changes. |
| 191b | `background-size` | New end-of-enum libcss property (`CSS_PROP_BACKGROUND_SIZE = 0x08D`, `int32_t background_size` at end of `_i`). Parser accepts `auto`, `cover`, `contain`, `<length>` 1/2-value forms. Consumer in `html_redraw_background` resolves tile dimensions and overrides `bg_data.width/height` before `content_redraw`. Bitmap-only V1; gradient-bg tiling deferred. Per the CLAUDE.md fixes151b padding gotcha: storage uses `int32_t` self-aligning end-of-struct field. dispatch.c + s_dispatch.c kept in sync per fixes116b. |
| 191c | `position: sticky` | Consumer-only round; `CSS_POSITION_STICKY` already parses. In `html_redraw_box`, before computing `(x, y) = (x_parent + box->x, y_parent + box->y)`, adjust `y_parent` upward so the painted y never goes above `top:` in viewport coords. Layout size unchanged. V1 limits: vertical only, no parent-bottom clamp, no nested scroll containers. |
| 191d | `object-position` | cssh_css drop. `apply_object_fit` already centres the fitted image — matches `center center` default. Non-default values silently degrade to centre; PARTIAL. |
| 191e | `pointer-events` | cssh_css drop. Hit-test still treats every visible box as targettable; PARTIAL. Real consumer requires a new libcss property and is deferred. |
| 191f | `transition`, `animation`, `@keyframes`, plus several CSS3 UI knobs (`user-select`, `overscroll-behavior-*`, `will-change`, `contain`, `content-visibility`, `scroll-margin-*`, `scroll-snap-*`, `font-display`, etc.) | cssh_css `modern_compat` DROP_PROPS list extended. Final static value of each transitioned property still applies via the normal cascade; no timer playback. `@keyframes` blocks parse but stay inert because no `animation-name` is in the cascade. |

Files touched: `cssh_css.c` (preprocessor extensions), `redraw.c`
(sticky + bg-size consumer), `propstrings.h` / `propstrings.c`,
`autogenerated_computed.h`, `computed.h`/`computed.c`, `dispatch.c`,
`s_dispatch.c`, `properties.h` (both libcss and select-side),
`properties.c` (handler array + unit mask), plus two new files
`p_background_size.c` and `s_background_size.c`. Tests under
`tests/css/`: `inset.html`, `background_size.html`, `sticky.html`,
`object_position.html`, `pointer_events.html`, `transition_static.html`.

Mac-side action: add `p_background_size.c` and `s_background_size.c`
to MacSurf.mcp.
