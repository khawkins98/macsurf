 ...
 * against the updated autogenerated_computed.h. css_select.c
 * allocates / initialises css_computed_style and must see the
 * new struct size (with macsurf_justify appended post-z_index).
 * Without this recompile the allocation is short and writes to
 * the new field land in adjacent heap memory.
 */
