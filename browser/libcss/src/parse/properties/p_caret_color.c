 ...
 *
 * Storage in css_computed_style_i.caret_color_status (int32_t):
 *   0 = INHERIT
 *   1 = AUTO
 *   2 = CURRENTCOLOR
 *   3 = SET (use caret_color word as a css_color)
 */
		...
		css_style *result)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) &&
		...
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_CARET_COLOR, 0, CARET_COLOR_AUTO);
	} else if ((token->type == CSS_TOKEN_IDENT) &&
			(lwc_string_caseless_isequal(
		...
		*ctx = orig_ctx;
		...
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_CARET_COLOR, 0,
				value == COLOR_SET ? CARET_COLOR_SET : value);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
