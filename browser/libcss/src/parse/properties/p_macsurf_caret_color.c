 ...
 *
 * Parse -macsurf-caret-color.
 *
 * The cssh_css.c preprocessor rewrites `caret-color: VALUE` to
 * `-macsurf-caret-color: VALUE` so author CSS reaches this parser
 ...
 *   css_color in css_computed_style_i.macsurf_caret_color.
 *
		...
		css_style *result)
{
	...
	if (token->type == CSS_TOKEN_IDENT &&
		...
		if (idata[0] == 'a' && idata[1] == 'u' &&
			...
			error = css__stylesheet_style_appendOPV(result,
					CSS_PROP_MACSURF_CARET_COLOR, 0,
					0x0080 /* SET */);
			if (error != CSS_OK) {
				*ctx = orig_ctx;
				return error;
	...
	error = css__stylesheet_style_appendOPV(result,
			CSS_PROP_MACSURF_CARET_COLOR, 0, 0x0080 /* SET */);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
